...
resource "aws_cloudfront_distribution" "cloud-front" {
  ...
  origin {
    domain_name = aws_route53_record.record_a_alb.fqdn
  ...
  origin {
    ...
    s3_origin_config {
      origin_access_identity = aws_cloudfront_origin_access_identity.cloud_front_oai.cloudfront_access_identity_path
...
# OAI
resource "aws_cloudfront_origin_access_identity" "cloud_front_oai" {
...
resource "aws_route53_record" "route53_cloudfront" {
  ...
  alias {
    name                   = aws_cloudfront_distribution.cloud-front.domain_name
    zone_id                = aws_cloudfront_distribution.cloud-front.hosted_zone_id
